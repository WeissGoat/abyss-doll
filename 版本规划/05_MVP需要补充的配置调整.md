...
### 2.1 一阶动力核心改为 Boss 奖励表保底配置

**当前问题：** 【一阶动力核心】位于 `elite_scrap_guard` 的普通 `LootPool` 中，不符合文档中的“必掉”定位。后续奖励来源会扩展到宝箱、事件和任务，因此不建议继续在怪物配置里直接维护复杂掉落池。

**调整建议：**

`elite_scrap_guard` 只配置奖励表 ID：
...
新增 `配置表(JSON)/Rewards/reward_monster_elite_scrap_guard.json`：
...
**当前问题：** 义体配置曾使用 `PassiveEffect` 简写字段，而运行时效果系统应统一消费 `EffectData` 列表。后续不保留 `PassiveEffect` / `PassiveEffects` 兼容字段。
...
**备注：** 义体和物品效果统一使用 `EffectData`。义体的 `Target` 可直接填写物品 Tag，例如 `Melee`，用于筛选背包内生效物品。
...
1.  新增 `/Rewards` 奖励表，并让 `elite_scrap_guard` 通过 `RewardID` 指向 Boss 奖励表。
2.  `pros_power_arm` / `pros_cooling_system`：统一为 `Effects`。
